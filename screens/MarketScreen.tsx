...
export default function MarketScreen({ 
  ...
  }, [allTeams, userTeam.id, searchTerm]);
  ...
  const handleConfirmBid = () => {
    ...
    if (ratio >= 1.2 || (ratio >= 0.9 && Math.random() > 0.4)) {
      ...
      toast.error(`O ${negotiatingPlayer.t.shortName} recusou sua proposta por ${p.name}. Eles querem mais.`);
    }
  };
        ...
        {activeTab === 'EXPLORE' && (
          <div className="space-y-3">
             <input 
                ...
                className="w-full bg-surface border border-white/5 p-4 rounded-2xl text-sm outline-none focus:ring-1 ring-primary"
                value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
             />
             <div className="space-y-2">
                {availablePlayers.slice(0, 30).map(({p, t}) => (
